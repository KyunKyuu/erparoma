...
class SupplierController extends Controller
{
    ...
    public function insert(Request $request)
    {
        ...
        if ($check->count() > 0) {
            return response()->json(['message' => 'Gagal, Nama supplier telah ada. Coba lagi!', 'status' => 'error'], 500);
        }
        $request->request->add(['created_by' => 'admin']);
        Supplier::create($request->all());
        return response()->json(['message' => 'Selamat, supplier berhasil ditambahkan!', 'status' => 'success'], 200);
    }

    public function delete(Request $request)
    {
        Supplier::where('id', $request->id)->delete();
        return response()->json(['message' => 'Selamat, supplier berhasil dihapus!', 'status' => 'success'], 200);
    }
